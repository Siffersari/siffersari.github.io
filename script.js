...
function setupSpotifyPlayer() {
    window.onSpotifyWebPlaybackSDKReady = () => {
        ...
        player.addListener('initialization_error', ({ message }) => { showNotification(`Initialization Error: ${message}`); });
        player.addListener('authentication_error', ({ message }) => { showNotification(`Authentication Error: ${message}`); });
        player.addListener('account_error', ({ message }) => { showNotification(`Account Error: ${message}`); });
        player.addListener('playback_error', ({ message }) => { showNotification(`Playback Error: ${message}`); });

        
        player.addListener('player_state_changed', state => {
            if (!state) return;
            isPlaying = !state.paused;
            playPauseBtn.querySelector('i').classList = isPlaying ? 'fas fa-pause' : 'fas fa-play';
        });
...
async function playSong() {
    if (!accessToken || !player || !deviceId || tracks.length === 0) {
        showNotification('Unable to play song. Please check your connection and try again.');
        return;
    }
...
async function pauseSong() {
    if (!accessToken || !player || !deviceId) {
        showNotification('Unable to pause song. Please check your connection and try again.');
        return;
    }
